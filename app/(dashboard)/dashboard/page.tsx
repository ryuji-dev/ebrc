...
export default async function DashboardPage() {
  ...
  let activePlansCount = planIds.length;
  ...
  if (planIds.length > 0) {
    ...
    const totalChapters = userPlans?.reduce((acc, p) => acc + (p.reading_plans as any).total_chapters, 0) || 0;
    overallPlanProgress = totalChapters > 0 ? Math.round((completedCount || 0) / totalChapters * 100) : 0;
  }
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-1000">
      ...
      <div className="relative overflow-hidden rounded-[2.5rem] bg-gradient-to-br from-primary/20 via-primary/5 to-transparent border border-white/5 p-6 md:p-10">
        <div className="relative z-10 space-y-3">
          <h1 className="text-2xl md:text-4xl text-white font-medium tracking-tight leading-tight">
            안녕하세요, <br className="md:hidden" /><span className="text-primary font-semibold">{profile?.full_name || '사용자'}</span>님!
          </h1>
          <p className="text-zinc-400 text-base md:text-lg font-medium max-w-2xl">
            오늘도 하나님과 깊이 교제하며 성령 충만한 하루 보내시길 기도합니다.
          </p>
        </div>
        <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[120%] bg-primary/10 blur-[120px] rounded-full px-4" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
        <Card className="glass-dark border-white/10 p-8 flex flex-col justify-between min-h-[250px] rounded-[2.5rem] relative overflow-hidden group hover:border-primary/40 transition-all duration-500">
          <div className="relative z-10">
            <h3 className="text-3xl text-white tracking-wide font-medium">경건의 기록</h3>
          ...
          <Link href="/devotion" className="mt-8 relative z-10">
            <Button size="2xl" className="w-full transition-all bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/20">
        ...
        <Card className="glass-dark border-white/10 p-8 flex flex-col justify-between min-h-[250px] rounded-[2.5rem] relative overflow-hidden group hover:border-white/20 transition-all duration-500">
          <div className="relative z-10">
            <h3 className="text-3xl text-white tracking-wide font-medium">성경 통독표</h3>
            <p className="text-zinc-500 mt-3 text-lg font-medium">연간 계획을 따라 성경을 완독하세요</p>
          </div>
          <Link href="/reading" className="mt-8 relative z-10">
            <Button variant="glass" size="2xl" className="w-full transition-all border-white/10 hover:bg-white/5 text-white">
              통독표 보기
            </Button>
          </Link>
